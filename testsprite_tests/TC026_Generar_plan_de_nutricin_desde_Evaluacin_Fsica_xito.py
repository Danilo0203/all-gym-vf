...
async def run_test():
    async with async_playwright() as p:
        ...
        await page.get_by_label("Peso (kg)").fill("70")
        await page.get_by_label("Estatura (cm)").fill("170")
        await page.get_by_role("button", name="Guardar").click()

        await expect(page.get_by_text("Evaluación física registrada").first).to_be_visible(timeout=10000)
        await browser.close()
